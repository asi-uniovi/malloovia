...
def solutions_from_dict(data: Mapping[str, Any], yaml_filename: str) -> Mapping[str, Union[SolutionI, SolutionII]]:
    """Takes data from a dictionary with a particular structure, and stores it in
    ...
    def is_phase_i_solution(solution_dict):
    ...
    def create_phase_i_solution(solution_dict):
        return SolutionI(**solution_dict)

    def create_phase_ii_solution(solution_dict):
        solution_dict['previous_phase'] = ids_to_objects[id(solution_dict['previous_phase'])]
        return SolutionII(**solution_dict)

    def dict_list_to_id_list(dict_list):
        id_list = []
        for item in dict_list:
            id_list.append(ids_to_objects[id(item)])
        return id_list

    def convert_allocation(solution_dict):
        allocation = solution_dict['allocation']
        solution_dict['allocation']['apps'] = dict_list_to_id_list(allocation['apps'])
        solution_dict['allocation']['instance_classes'] = dict_list_to_id_list(allocation['instance_classes'])

    def create_solution(solution_dict):
        solution_dict['problem'] = ids_to_objects[id(solution_dict['problem'])]

        if 'allocation' in solution_dict:
            convert_allocation(solution_dict)

        if is_phase_i_solution(solution_dict):
            res_alloc = solution_dict['reserved_allocation']
            res_alloc['instance_classes'] = dict_list_to_id_list(res_alloc['instance_classes'])
            result = create_phase_i_solution(solution_dict)
        else:
            result = create_phase_ii_solution(solution_dict)

        ids_to_objects[id(solution_dict)] = result
        return result

    problems, ids_to_objects = _problems_and_ids_from_dict(data, yaml_filename)
    ...
    for solution_dict in data['Solutions']:
        if is_phase_i_solution(solution_dict):
            solution = create_solution(solution_dict)
            solutions[solution.id] = solution

     # Create solutions for phase II
    for solution_dict in data['Solutions']:
        if not is_phase_i_solution(solution_dict):
            solution = create_solution(solution_dict)
            solutions[solution.id] = solution
